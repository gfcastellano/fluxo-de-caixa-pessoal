...
- multi-currency handling (already exists in accounts)

## 4) Current implementation notes (HOW it works today)
### 4.1 Backend routes
Main API mounts:
- /api/transactions, /api/accounts, /api/credit-cards, /api/credit-card-bills, /api/reports, /api/voice, /api/families, etc.

### 4.2 Voice pipeline (backend)
- Client uploads audio as multipart/form-data.
- Backend transcribes with Whisper.
- Backend parses with GPT and matches:
  - categories
  - accounts (incl. cash flag)
  - credit cards
- For updates:
  - persists to Firestore (update existing tx)
- For “pending updates”:
  - returns updated draft (no persistence)
...
Any money math must be isolated (pure functions):

installments, bills, recurring generation, projections
