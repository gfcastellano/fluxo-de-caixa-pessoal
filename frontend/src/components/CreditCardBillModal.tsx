...
import type { CreditCardBill, Transaction, Account, CreditCard, Category } from '../types';
...
export function CreditCardBillModal({
  ...
  const loadData = async () => {
    ...
    try {
      const [billTransactions, userAccounts, userCategories] = await Promise.all([
      ...
      // Propose a category for bill payment if available
      const billCategory = userCategories.find(c =>
      ...
      if (billCategory) {
        setSelectedCategoryId(billCategory.id);
      }
  ...
  const handlePayBill = async () => {
    if (!bill || !selectedAccountId || !selectedCategoryId) return;
    ...
    setLoading(true);
    try {
      await payBill(bill.id, selectedAccountId, selectedCategoryId);
      onBillUpdated();
      onClose();
    } catch (err) {
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    <option value="">{t('creditCardBills.selectAccount') || 'Selecione'}</option>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue/20"
                  >
                    <option value="">{t('common.selectCategory') || 'Selecione'}</option>
                    {categories
              ...
              <button
                ...
                disabled={loading || !selectedAccountId || !selectedCategoryId}
