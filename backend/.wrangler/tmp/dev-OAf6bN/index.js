...
// .wrangler/tmp/bundle-a5pnbZ/strip-cf-connecting-ip-header.js
...
Response must be valid JSON only, no markdown, no explanation.`;
   ...
   */
  async parseTransactionUpdate(transcription, currentTransaction, categories, language = "en") {
    const now = /* @__PURE__ */ new Date();
    const today = now.toISOString().split("T")[0];
    const currentYear = now.getFullYear();
    const currentMonth = now.getMonth() + 1;
...
// .wrangler/tmp/bundle-a5pnbZ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-a5pnbZ/middleware-loader.entry.ts
