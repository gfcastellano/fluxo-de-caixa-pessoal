...
// .wrangler/tmp/bundle-7cjxWa/strip-cf-connecting-ip-header.js
...
var FirebaseService = class {
  ...
  async getDocuments(collection, userId) {
    ...
    return documents.filter(
      (doc) => doc.fields?.userId?.stringValue === userId
    ).map(
      (doc) => this.convertFromFirestore(doc)
    );
  }
  ...
  convertToFirestore(data) {
    ...
    for (const [key, value] of Object.entries(data)) {
      if (value === null) {
        ...
        result[key] = { mapValue: { fields: this.convertToFirestore(value) } };
      }
    }
    return result;
  }
  ...
  parseFirestoreValue(value) {
    ...
    if ("mapValue" in v && v.mapValue && typeof v.mapValue === "object") {
      const mapValue = v.mapValue;
      return mapValue.fields ? this.convertFromFirestore({ name: "", fields: mapValue.fields }) : {};
    }
    if ("arrayValue" in v && v.arrayValue && typeof v.arrayValue === "object") {
      const arrayValue = v.arrayValue;
      return arrayValue.values ? arrayValue.values.map((item) => this.parseFirestoreValue(item)) : [];
    }
    return value;
  }
...
__name(FirebaseService, "FirebaseService");
...
var transactionSchema = external_exports.object({
  ...
  isRecurring: external_exports.boolean().optional(),
  recurrencePattern: external_exports.enum(["monthly", "weekly", "yearly"]).nullable().optional(),
  recurrenceDay: external_exports.number().nullable().optional(),
  recurrenceEndDate: external_exports.string().nullable().optional(),
  parentTransactionId: external_exports.string().nullable().optional()
});
var app2 = new Hono2();
...
app2.post("/", async (c) => {
  try {
    ...
    if (validated.isRecurring && validated.recurrencePattern) {
      await generateRecurringInstances(firebase, transaction, validated);
    }
...
app2.post("/:id/generate-recurring", async (c) => {
  try {
    const transactionId = c.req.param("id");
    const userId = c.get("userId");
    const firebase = new FirebaseService(c.env);
    ...
    const instances = await generateRecurringInstances(firebase, transaction, transaction);
...
async function generateRecurringInstances(firebase, parentTransaction, data) {
  const instances = [];
  const startDate = new Date(data.date);
  const currentYear = (/* @__PURE__ */ new Date()).getFullYear();
  const endOfYear = new Date(currentYear, 11, 31);
  const endDate = data.recurrenceEndDate ? new Date(Math.min(new Date(data.recurrenceEndDate).getTime(), endOfYear.getTime())) : endOfYear;
  ...
  while (currentDate <= endDate) {
    ...
    const instance = await firebase.createDocument("transactions", {
      ...
      isRecurring: false,
      // Instances are not recurring themselves
      createdAt: (/* @__PURE__ */ new Date()).toISOString(),
      updatedAt: (/* @__PURE__ */ new Date()).toISOString()
    });
...
function getNextDate(currentDate, pattern, recurrenceDay) {
  ...
  switch (pattern) {
    ...
    case "monthly":
      ...
      const targetDay = recurrenceDay !== null && recurrenceDay !== void 0 ? recurrenceDay : currentDate.getDate();
...
__name(getNextDate, "getNextDate");
...
var budgetSchema = external_exports.object({
  categoryId: external_exports.string(),
  amount: external_exports.number().positive(),
  period: external_exports.enum(["monthly", "yearly"]),
...
var budgets_default = app3;

// src/routes/reports.ts
var reportQuerySchema = external_exports.object({
  year: external_exports.string().transform(Number),
  month: external_exports.string().transform(Number)
});
var app4 = new Hono2();
app4.use("*", authMiddleware);
app4.get("/monthly-summary", async (c) => {
  try {
    const userId = c.get("userId");
...
app4.get("/category-breakdown", async (c) => {
  try {
    const userId = c.get("userId");
    const query = c.req.query();
    const { year, month } = reportQuerySchema.parse(query);
    ...
    const [transactions, categories] = await Promise.all([
      firebase.getDocuments("transactions", userId),
      firebase.getDocuments("categories", userId)
    ]);
    const startDate = `${year}-${String(month).padStart(2, "0")}-01`;
    const endDate = `${year}-${String(month).padStart(2, "0")}-31`;
    const filteredTransactions = transactions.filter(
      (t) => t.date >= startDate && t.date <= endDate && t.type === type
    );
...
var OpenAIService = class {
   ...
   */
  async parseTransaction(transcription, categories, language = "en", defaultDescription = "Voice transaction", accounts = []) {
...
- "Gast\xE9 50 euros en supermercado" (es) \u2192 {"amount": 50, "type": "expense", "description": "Supermercado", "categoryId": "...", "date": "${today}", "accountId": "", "isRecurring": false, "recurrencePattern": null, "recurrenceDay": null, "recurrenceEndDate": null}
...
Response must be valid JSON only, no markdown, no explanation.`;
   ...
   */
  async parseTransactionUpdate(transcription, currentTransaction, categories, language = "en", accounts = []) {
    const now = /* @__PURE__ */ new Date();
    const today = now.toISOString().split("T")[0];
    const currentYear = now.getFullYear();
    const currentMonth = now.getMonth() + 1;
...
Response must be valid JSON only, no markdown, no explanation.`;
    ...
    try {
      const parsed = JSON.parse(jsonString);
      const updates = {};
      if (parsed.amount !== void 0 && typeof parsed.amount === "number" && parsed.amount > 0) {
        updates.amount = parsed.amount;
      }
      if (parsed.type !== void 0 && ["income", "expense"].includes(parsed.type)) {
        updates.type = parsed.type;
      }
      if (parsed.description !== void 0 && typeof parsed.description === "string" && parsed.description.trim()) {
        updates.description = parsed.description.trim();
      }
      if (parsed.categoryId !== void 0 && typeof parsed.categoryId === "string") {
        updates.categoryId = parsed.categoryId;
      }
      if (parsed.date !== void 0 && typeof parsed.date === "string") {
   ...
   */
  async parseCategory(transcription, language = "en") {
    const languageInstructions = {
      en: "Return the name in English",
      pt: "Return the name in Portuguese (Portugu\xEAs)",
      es: "Return the name in Spanish (Espa\xF1ol)"
    };
    const langInstruction = languageInstructions[language] || languageInstructions.en;
...
- "Nova categoria sal\xE1rio" \u2192 {"name": "Sal\xE1rio", "type": "income", "color": "#4ECDC4"}
- "Nueva categor\xEDa entretenimiento" \u2192 {"name": "Entretenimiento", "type": "expense", "color": "#FFE66D"}
...
Response must be valid JSON only, no markdown, no explanation.`;
   ...
   */
  async parseBudget(transcription, categories, language = "en") {
    const now = /* @__PURE__ */ new Date();
    const today = now.toISOString().split("T")[0];
    const categoryList = categories.map((c) => `"${c.name}" (ID: ${c.id}, Type: ${c.type})`).join("\n");
    const periodExamples = {
      ...
      pt: '- "Or\xE7amento mensal de 1000 reais para transporte" \u2192 {"categoryId": "...", "amount": 1000, "period": "monthly", "startDate": "${today}", "matchedCategory": "Transport"}\n- "Or\xE7amento anual de 5000 para f\xE9rias" \u2192 {"categoryId": "...", "amount": 5000, "period": "yearly", "startDate": "${today}", "matchedCategory": "Vacation"}',
...
Examples:
${examples}
- "Or\xE7amento de 500 para Pet Shop" (when "Pet Shop" exists) \u2192 {"categoryId": "...", "amount": 500, "period": "monthly", "startDate": "${today}", "matchedCategory": "Pet Shop"}
- "Or\xE7amento de 500 para XYZ" (when "XYZ" doesn't exist) \u2192 {"categoryId": "", "amount": 500, "period": "monthly", "startDate": "${today}", "matchedCategory": null}
...
Response must be valid JSON only, no markdown, no explanation.`;
   ...
   */
  async parseBudgetUpdate(transcription, currentBudget, categories, language = "en") {
    const now = /* @__PURE__ */ new Date();
    const today = now.toISOString().split("T")[0];
    const categoryList = categories.map((c) => `"${c.name}" (ID: ${c.id}, Type: ${c.type})`).join("\n");
    const periodExamples = {
      ...
      pt: '- "Mude o per\xEDodo para anual" \u2192 {"period": "yearly"}\n- "Altere o valor para 1000" \u2192 {"amount": 1000}\n- "Mude a categoria para transporte" \u2192 {"categoryId": "...", "matchedCategory": "Transport"}',
      es: '- "Cambie el per\xEDodo a anual" \u2192 {"period": "yearly"}\n- "Actualice el monto a 1000" \u2192 {"amount": 1000}\n- "Cambie la categor\xEDa a transporte" \u2192 {"categoryId": "...", "matchedCategory": "Transport"}'
    };
    const examples = periodExamples[language] || periodExamples.en;
...
Response must be valid JSON only, no markdown, no explanation.`;
    ...
    try {
      ...
      if (parsed.period !== void 0 && ["monthly", "yearly"].includes(parsed.period)) {
        updates.period = parsed.period;
      }
      if (parsed.startDate !== void 0 && typeof parsed.startDate === "string") {
        updates.startDate = parsed.startDate;
      }
      if (parsed.matchedCategory !== void 0) {
   ...
   */
  async parseAccount(transcription, language = "en") {
    const languageInstructions = {
      en: "Return the name in English",
      pt: "Return the name in Portuguese (Portugu\xEAs)",
      es: "Return the name in Spanish (Espa\xF1ol)"
...
// src/i18n/voice-translations.ts
var voiceTranslations = {
  en: {
    errors: {
  ...
  pt: {
    errors: {
      noAudioFile: "Nenhum arquivo de \xE1udio fornecido",
      emptyAudio: "Arquivo de \xE1udio vazio",
      transcriptionFailed: "Falha ao transcrever \xE1udio",
      noSpeechDetected: "Nenhuma fala detectada",
      parsingFailed: "N\xE3o foi poss\xEDvel entender a transa\xE7\xE3o. Tente novamente.",
      processingFailed: "Falha ao processar transa\xE7\xE3o por voz",
      categoryNotFound: "Categoria n\xE3o encontrada. Por favor, crie a categoria primeiro."
    },
    success: {
      transactionCreated: "Transa\xE7\xE3o criada com sucesso",
      transactionUpdated: "Transa\xE7\xE3o atualizada com sucesso",
      categoryCreated: "Categoria processada com sucesso",
      budgetCreated: "Or\xE7amento processado com sucesso",
      budgetUpdated: "Or\xE7amento atualizado com sucesso",
      accountCreated: "Conta processada com sucesso"
    },
    defaults: {
      description: "Transa\xE7\xE3o por voz"
    }
  },
  es: {
    errors: {
      noAudioFile: "No se proporcion\xF3 archivo de audio",
      emptyAudio: "Archivo de audio vac\xEDo",
      transcriptionFailed: "Error al transcribir el audio",
      noSpeechDetected: "No se detect\xF3 voz",
      parsingFailed: "No se pudo entender la transacci\xF3n. Int\xE9ntalo de nuevo.",
      processingFailed: "Error al procesar la transacci\xF3n por voz",
      categoryNotFound: "Categor\xEDa no encontrada. Por favor, crea la categor\xEDa primero."
    },
    success: {
      transactionCreated: "Transacci\xF3n creada exitosamente",
      transactionUpdated: "Transacci\xF3n actualizada exitosamente",
      categoryCreated: "Categor\xEDa procesada exitosamente",
    ...
    defaults: {
      description: "Transacci\xF3n por voz"
    }
  }
};
function getErrorTranslation(language, key) {
...
__name(getErrorTranslation, "getErrorTranslation");
function getSuccessTranslation(language, key) {
  const translations = voiceTranslations[language] || voiceTranslations.en;
  return translations.success[key] || voiceTranslations.en.success[key];
}
__name(getSuccessTranslation, "getSuccessTranslation");
function getDefaultTranslation(language, key) {
  const translations = voiceTranslations[language] || voiceTranslations.en;
  return translations.defaults[key] || voiceTranslations.en.defaults[key];
}
__name(getDefaultTranslation, "getDefaultTranslation");

// src/routes/voice.ts
var app5 = new Hono2();
app5.use("*", authMiddleware);
app5.post("/transactions/update", async (c) => {
  let language = "en";
  try {
    const userId = c.get("userId");
    ...
    const currentTransactionStr = formData.get("currentTransaction");
    ...
    const currentTransaction = JSON.parse(currentTransactionStr);
...
app5.post("/transactions", async (c) => {
  try {
    ...
    try {
      const formData = await c.req.formData();
      errorLanguage = formData.get("language") || "en";
    } catch {
    }
    return c.json(
...
app5.post("/budgets", async (c) => {
  ...
  try {
    ...
    const currentBudgetStr = formData.get("currentBudget");
    ...
    if (isEditing && currentBudgetStr) {
      currentBudget = JSON.parse(currentBudgetStr);
    }
    const openai = new OpenAIService(c.env);
    let transcription;
    try {
    ...
    return c.json({
      ...
      data: parsedBudget,
      transcription,
      message: isEditing ? getSuccessTranslation(language, "budgetUpdated") : getSuccessTranslation(language, "budgetCreated")
    }, 200);
  } catch (error) {
...
var accountSchema = external_exports.object({
  ...
  initialBalance: external_exports.number(),
    ...
    if (isDefault && !isFirstAccount) {
      ...
      for (const account2 of accounts) {
        if (account2.isDefault) {
          await firebase.updateDocument("accounts", account2.id, { isDefault: false });
        }
      }
    }
...
app6.delete("/:id", async (c) => {
  try {
    ...
    const isDefault = existingAccount.isDefault;
    if (isDefault) {
...
app6.patch("/:id/set-default", async (c) => {
  try {
    const accountId = c.req.param("id");
    const userId = c.get("userId");
    const firebase = new FirebaseService(c.env);
...
app7.route("/api/categories", categories_default);
app7.route("/api/transactions", transactions_default);
app7.route("/api/budgets", budgets_default);
app7.route("/api/reports", reports_default);
app7.route("/api/voice", voice_default);
app7.route("/api/accounts", accounts_default);
app7.notFound((c) => {
...
// .wrangler/tmp/bundle-7cjxWa/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-7cjxWa/middleware-loader.entry.ts
